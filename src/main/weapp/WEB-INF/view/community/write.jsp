...
/* 		$("#displayFilename").change(function(){
		...
		$("#writeBtn").click(function(){
			...
			writeForm.attr({
				"method" : "post",
				"action" : url//클릭했을때 포스트 방식으로 write 주소로 전송
			});
			writeForm.submit();
		});
	...
	<div>
		...
		<form:form modelAttribute="writeFrom" enctype="multipart/form-data"> 
			...
			<c:if test="${mode == 'modify' && 
				...
				<div>
					<input type="checkbox" id="displayFilename" 
		...
		<%-- 	<p>
				날짜 : <input type="date" id="writeDate" name="writeDate" value="${communityVO.writeDate}"/>
			</p>
			<div>
				<form:errors path="writeDate"/><!--체크하고자하는 엘리먼트의 name을 적어준다.  -->
			</div> --%>
